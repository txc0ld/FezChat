...
///
/// Connects to `wss://relay.festichat.app/ws` and sends/receives the same
...
public final class WebSocketTransport: NSObject, Transport, @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.festichat.websocket", qos: .userInitiated)
    private let logger = Logger(subsystem: "com.festichat", category: "WebSocket")
    ...
    public init(localPeerID: PeerID, noisePublicKey: Data) {
        ...
        self.serverPeerID = PeerID(noisePublicKey: Data("relay.festichat.app".utf8))
