...
public struct PeripheralState: Sendable {
    ...
    /// The FestiChat PeerID derived from the peer's Noise public key.
...
public final class PeerManager: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.festichat", category: "PeerManager")
    private var evaluationTimer: DispatchSourceTimer?
    private let queue = DispatchQueue(label: "com.festichat.peerManager", qos: .utility)
