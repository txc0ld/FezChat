...
/// 3. Notify via `onSendFailed` (app layer persists for retry via SwiftData)
///
...
public final class TransportCoordinator: @unchecked Sendable, Transport {
    ...
    // MARK: - Send failure callback

    /// Called when both transports fail and the message cannot be sent.
    /// The app layer can persist the message for retry via SwiftData.
    public var onSendFailed: ((_ data: Data, _ targetPeer: PeerID?) -> Void)?
    ...
    ///
    /// Tries BLE first, then WebSocket, then calls `onSendFailed` if both fail.
    ///
    /// - Parameters:
    ...
    public func send(data: Data, to peerID: PeerID) {
        ...
        // Notify app layer so it can persist for retry via SwiftData.
