...
2. **File tickets** in Notion with full Claude Code prompts in the body.
...
5. **Merge** PRs with squash + conventional commit (you have the GitHub PAT). Then verify on `main` and transition the Notion ticket to Fixed.
...
1. **Set your handle.** Ask John for one — `claude-pm-1` is the convention if he doesn't give you one. Use it on Notion `Owner` claims and as the `from` field in Slack posts (well — the Blip bot is the `from`, but reference your handle in your messages).
...
3. **Read the rules**, in this order:
   ...
   - `docs/PM/memory/reference_notion_workspace.md` — Notion API patterns, schemas, IDs
...
5. **Read the live state.**
   ...
   - **Notion hub**: https://www.notion.so/HeyBlip-34c3e435f07a80acbe11e76655af9ebf — read the 🤖 callout + the Tasks DB. The Default view is filtered to open work and sorted Status ascending (New → Closed).
...
|---|---|---|
| Code | `https://github.com/txc0ld/heyblip` (app), `https://github.com/txc0ld/heyblip.au` (website) | `gh` CLI with `GITHUB_PAT` |
| Tasks | Notion HeyBlip workspace, Tasks DB `34c3e435-f07a-8175-bbdd-e0c455d106f7` | REST API with `NOTION_TOKEN` |
| Decisions | Same Notion workspace, Decisions DB `34c3e435-f07a-8178-812d-cc6e712d32e9` | Same |
| Components | Same workspace, Components DB `34c3e435-f07a-811b-902b-fd802b1518e4` | Same |
| Bugasura archive | https://my.bugasura.io/HeyBlip — **read-only**, all new edits in Notion | Via web UI; API still works for read-only lookups with `BUGASURA_API_KEY` |
...
## What's open right now (2026-04-24 EOD snapshot — verify before acting)

### Backlog priorities

- **HEY-1192** (HIGH, In Progress) — PUSH-5 two-phone smoke test on build 30 / 31. Closes the HEY-1187 push notifications epic. Needs John + 2 phones + 30-60 min hands-on.
...
- Add the Cowork integration to the Notion HeyBlip workspace if you want the Notion MCP to work via OAuth instead of the personal token (one-click in Notion settings → Connections).
...
Anything filed AFTER this snapshot is in Notion. Trust Notion over this doc for ticket state.

## Don'ts (carryover from prior sessions)

- Don't auto-dispatch from the Notion `Assigned to` column — there's no worker watching it. All dispatch is manual on John's direction.
...
- Don't write to Notion `Status`, `Approved to merge`, or `Closed` fields when acting AS an engineer agent. PM/Cowork DOES manage those — that's your role here.
