...
/// Supports Dynamic Type scaling for accessibility via `.relativeTo:` anchors.
struct BlipTypography: Sendable {

    /// Hero / display — Bold 40pt
    let display: Font

    /// Large titles — Bold 34pt
    let largeTitle: Font

    /// Secondary titles — Bold 28pt
    let title2: Font

    /// Tertiary titles — SemiBold 20pt
    let title3: Font
    ...
    /// Callout / inline emphasis — Medium 16pt
    let callout: Font
    ...
    /// Smallest captions (badges, map labels) — SemiBold 9pt
    ...
    static let standard = BlipTypography(
        ...
        captionSmall: .custom(BlipFontName.semiBold, size: 9, relativeTo: .caption2)
    )
    ...
    static let system = BlipTypography(
        ...
        captionSmall: .system(size: 9, weight: .semibold, design: .default)
    )
}
